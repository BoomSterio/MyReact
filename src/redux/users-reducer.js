...
export const toggleFollowingProgress = (isFollowingInProgress, userId) => ({type: TOGGLE_FOLLOWING_IN_PROGRESS, isFollowingInProgress, userId})
...
export const follow = (userId) => {
    return (dispatch) => {
        ...
        usersAPI.postFollow(userId).then(responce => {
...
export const unfollow = (userId) => {
    return (dispatch) => {
        dispatch(toggleFollowingProgress(true, userId));
        usersAPI.deleteFollow(userId).then(responce => {
            if (responce.data.resultCode === 0) {
